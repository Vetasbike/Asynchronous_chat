...
#     Должно получиться четыре списка — например, os_prod_list, os_name_list, os_code_list, os_type_list. В этой же функции создать
#     главный список для хранения данных отчета — например, main_data — и поместить в него названия столбцов отчета в виде списка:
#     «Изготовитель системы», «Название ОС», «Код продукта», «Тип системы». Значения для этих столбцов также оформить в виде списка
...
def collect_data():
    data_dir = os.path.join(CURRENT_DIR, '/home/vitaly/Рабочий стол/AsynchronousChat/Asynchronous_chat/lesson_2')
    result = []
...
if __name__ == '__main__':
    write_to_csv('/home/vitaly/Рабочий стол/AsynchronousChat/Asynchronous_chat/lesson_2/new_data_report.csv')
